...
void Lux::Core::FreeLookCamera::Update(const float a_DeltaTime)
{
	...
	if (m_EventListener->GetMouseButtonDown(LUX_MOUSE_BUTTON_RIGHT))
	{
		...
		m_Transform->SetLocalRotation(angleAxis(m_EulerRotation.x, vec3(1, 0, 0)) * angleAxis(m_EulerRotation.y, vec3(0, 1, 0)) * angleAxis(m_EulerRotation.z, vec3(0, 0, 1)));
	}

	if (m_EventListener->GetKeyDown(KeyCode::LUX_KEY_W))
	{
		m_Transform->Translate(m_Transform->GetForwardVector() * m_SpeedMultiplier * a_DeltaTime);
	}
	...
	{
		m_Transform->Translate(-m_Transform->GetRightVector() * m_SpeedMultiplier * a_DeltaTime);
	}
